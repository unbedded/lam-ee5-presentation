...
- Hold power = 192 dots × 250µJ × 45Hz × 20% = 0.43W continuous
- This is the REALISTIC value based on partial refresh, not worst-case 100% recharge

SOLENOID - Inductive Load (L = 10 mH, R = 26 Ω, 12V drive):
- ACTUATION: Energy = ½Li² + i²Rt = 1.06mJ (magnetic) + 275mJ (heat loss) = 276 mJ total
- That's 1100× MORE energy per pulse than piezo! (276,000 µJ vs 250 µJ)
- 12V drive voltage (boosted from 5V USB) with 26Ω coil → 460mA current
...
- Duty cycle: 1.2s active / 6s total = 20% → Average power = 0.73W (includes 0.10W boost losses)

THE PARADOX RESOLVED:
- Solenoid uses 1100× more energy PER PULSE but fires RARELY (intermittent)
- Piezo uses 1/1100th energy PER PULSE but fires CONSTANTLY (45 Hz continuous)
- Result: 1.03W (piezo) vs 0.73W (solenoid) - only 1.4× difference
